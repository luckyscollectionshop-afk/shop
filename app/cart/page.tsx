...
export default async function CartPage() {
  ...
  const { data: cartItems, error: itemsError } = await supabase
    .from("cart_items")
    .select(
      ` id, quantity, product:products!cart_items_product_id_fkey( id, name, price, sale_price, images, active, stock ) `,
    )
    .eq("cart_id", cart.id)
    .order("created_at");
  if (itemsError) {
  ...
  const items = (cartItems ?? []).map((item) => ({
    ...
    product: Array.isArray(item.product)
      ? (item.product[0] ?? null)
      : item.product,
  })) as { id: string; quantity: number; product: CartProduct | null }[];
  ...
  return (
    <main className="min-h-screen bg-background">
      <SiteHeader isLoggedIn={true} isAdmin={isAdmin} cartCount={cartCount} />
            ...
            <div className="h-fit rounded-xl border p-5">
              ...
              <div className="mt-4 border-t pt-4">
                <div className="flex justify-between font-semibold">
                  <span>Total</span> <span>CHF {subtotal.toFixed(2)}</span>
                </div>
              </div>
